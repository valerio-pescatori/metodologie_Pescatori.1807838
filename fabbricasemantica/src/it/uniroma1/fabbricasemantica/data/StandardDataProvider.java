...
/**
 * Questa classe restituisce i dati necessari per il task ricevuto in input. Il
 * metodo {@link #getData(Task)} restituira' una stringa in formato JSON che le
 * pagine in JSweet andranno a formattare. N.B. Il proprio {@link DataProvider}
 * dovrebbe evitare di avere stringhe codificate, questo e' solo un esempio.
 */
public class StandardDataProvider implements DataProvider<String>
{
	...
	public String getData(Task task)
	{
		...
		{
			return "{" + "\"word\": \"car\"," + "\"hypernym\": \"motor_veichle\"" + "}";
		} else if (task == StandardTask.SENSE_ANNOTATION)
		{
			return "{" + "\"word\":\"mouse\"," + "\"description\":\"A swollen bruise caused by a blow to the eye\","
					+ "\"senses\": [\"Any of numerous small rodents typically resembling diminutive rats\", \"A hand-operated electronic device that controls the coordinates of a cursor on your computer screen\", \"person who is quiet or timid\", \"a swollen bruise caused by a blow to the eye\"]"
					+ "}";
